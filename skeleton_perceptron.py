...
DATA_PATH = "E:/School/@Grad School/Machine Learning/HW1/adult"
...
def perceptron(train_ys, train_xs, dev_ys, dev_xs, args):
    ...
    for i in range(args.iterations):
        for x, y in zip(train_xs, train_ys):
            pred_y = np.sign(np.dot(np.transpose(weights), x))

            if pred_y != y:
                weights += np.array(args.lr * x * y)

        if dev_xs is not None:
            acc = test_accuracy(weights, dev_ys, dev_xs)
            accuracy.append(acc)
            if acc > best_accuracy:
                best_accuracy, best_weights = acc, np.copy(weights)
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    pred_ys = [np.sign(np.dot(weights, x)) for x in test_xs]

    accuracy = sum(pred_ys == test_ys) / len(test_ys)
